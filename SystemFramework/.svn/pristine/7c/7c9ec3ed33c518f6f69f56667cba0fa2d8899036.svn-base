...
public class JdbcTemplate<T> {
	...
	public T queryForBean(String sql, Object... params) throws SQLException {
		...
		if (params != null && params.length > 0) {
			result = runer.query(con, sql, new BeanHandler<T>(this.clazz), params);
		} else {
			result = runer.query(con, sql, new BeanHandler<T>(this.clazz));
		}
	...
	public List<T> queryForBeanList(String sql, Object... params) throws SQLException {
		...
		if (params != null && params.length > 0) {
			list = (List<T>) runer.query(con, sql, new BeanListHandler<T>(this.clazz), params);
		} else {
			list = (List<T>) runer.query(con, sql, new BeanListHandler<T>(this.clazz));
		}
